...
const ProviderDashboardLayout = ({
  ...
  const marketplaceHref = providerProfile?.id ? `/proveedor/${providerProfile.id}` : "/buscar";
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <aside className="hidden lg:flex fixed inset-y-0 left-0 w-72 border-r border-slate-800 bg-slate-950 z-30">
        <div className="w-full p-5 flex flex-col">
          <Link to="/dashboard/proveedor" className="inline-flex items-center gap-2 text-slate-100 mb-7">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Panel de proveedor</p>
            </div>
          </Link>
      ...
      <Sheet open={sidebarOpen} onOpenChange={setSidebarOpen}>
        <SheetContent side="left" className="w-[85vw] max-w-xs p-0 bg-slate-950 border-r border-slate-800">
          <div className="p-5">
            <Link to="/dashboard/proveedor" className="inline-flex items-center gap-2 text-slate-100 mb-6" onClick={() => setSidebarOpen(false)}>
              <div className="h-9 w-9 rounded-xl bg-accent/20 border border-accent/40 flex items-center justify-center">
                <Building2 className="h-4 w-4 text-accent" />
              </div>
              <div>
