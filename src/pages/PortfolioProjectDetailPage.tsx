...
import { usePortfolioProject, usePhases } from "@/hooks/use-marketplace-data";
          ...
          <section className="mt-5 grid gap-3 rounded-xl border border-[#E3DDD4] bg-white p-4 md:grid-cols-[1fr_auto] md:items-center">
            ...
            <div className="flex flex-wrap gap-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/proveedor/${project.provider.id}`}>Ver perfil del proveedor</Link>
              </Button>
              <Button asChild size="sm">
                <Link to={`/proveedor/${project.provider.id}`}>Solicitar cotizacion</Link>
              </Button>
            </div>
          </section>
        </article>
